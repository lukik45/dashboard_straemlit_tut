...
import pandas as pd
import numpy as np
import plotly.graph_objects as go  # version of plotly that is highly adjustable
...
def style_negative(v, props=''):
    """Style negative values in a data frame"""
    try:
        return props if v < 0 else None
    except:
        pass  # if the value is not a number
    
def style_positive(v, props=''):
    """Style positive values in a data frame"""
    try:
        return props if v > 0 else None
    except:
        pass  # if the value is not a number

def audience_simple(country_code):
    """Show top countries"""
    if country_code == 'US':
        return 'USA'
...
@st.cache  # streamlit caches the data
def load_data():
...
df_agg_diff.iloc[:,numeric_cols] = (df_agg_diff.iloc[:,numeric_cols] - median_agg).div(median_agg)  # percent difference
...
df_time_diff_12mo = df_time_diff[df_time_diff['Video publish time'] >= date_12mo]


# get daily view data (first 30 days), median and percentiles

# pivot_table - aggregation
views_days = pd.pivot_table(df_time_diff_12mo, values='Views', index='days_published', aggfunc=
...
if add_sidebar  == 'Aggregate Metrics':
    ...
    for i in metric_medians_6mo.index:
        with columns[count]:
            ...
            st.metric(label=i, value = round(metric_medians_6mo[i],1), delta = '{:.2%}'.format(delta))
            count+=1
            if count >=5:
                count = 0
...
if add_sidebar  == 'Individual Video Analysis':
    # select box
    video_titles = tuple(df_agg['Video title'])
    video_select = st.selectbox('Pick a video', video_titles)
    ...
    fig2.add_trace(go.Scatter(x= first30['days_published'], y = first30['Views'].cumsum(),
                              mode= 'lines',
                              name = 'Current video',
                              line=dict(color='firebrick', width=5)))
    
    fig2.update_layout(title='View comparison for the first 30 days', xaxis_title= 'Days since publishing', yaxis_title= 'Cumulative views')
